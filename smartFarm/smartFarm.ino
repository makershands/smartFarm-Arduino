...
int val, cdsval;        // 조도 센서 값을 저장하기 위한 변수 선언
float temp, humi = 0.0; // 온습도 센서 값을 저장하기 위한 변수 선언

int waterpumpPower = 200; // 워터 펌프 모터의 기준값 지정
int fanPower = 200;       // 쿨링 팬의 기준값 지정
...
void loop() {
  ...
  psoil = map(soil, 0, 1023, 0, 100); // map함수를 사용하여 soil값을 1~100으로 변환한 값을 psoil에 저장
  val = analogRead(cds_pin);          // A1에서 읽은 값을 val 변수에 저장
  cdsval = map(val, 0, 1023, 100, 0); // map함수를 사용하여 val값을 1~100으로 변환한 값을 cdsval에 저장
  ...
  if (cdsval < 70) digitalWrite(led_pin, HIGH); // 조도 센서 값이 70% 미만이면 LED를 켬
